...
class DestinatarioControl {
    ...
    async eliminar(req, res) {
        try {
            ...
            const destinatarioEncontrado = await destinatario.findOne({
                where: { external_id: external },
            });
    
            if (!destinatarioEncontrado) {
                return res.status(404).json({ msg: "Destinatario no encontrado", code: 404 });
            }
    
            await destinatarioEncontrado.destroy();
    
            return res.status(200).json({ msg: "Destinatario eliminado correctamente", code: 200 });
        } catch (error) {
